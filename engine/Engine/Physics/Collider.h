...
struct  Collider {
    ...
    godot::Vector2 capsule_direction = godot::Vector2(1.f, 0.f);
    float capsule_half_length = 1.f;

    // plane collides when dot((normal).normalized(), p) - d < radius of the other body,
    // fixed in world space
    godot::Vector2 plane_normal = godot::Vector2(0.f, 1.f);
    float plane_d = 0.f;
};
