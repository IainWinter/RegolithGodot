...
func _handle_request(client: StreamPeerTCP, data: String) -> void:
	...
	match cmd:
		"game_view":
			# the Game tab's own toolbar: every button with its tooltip and
			# pressed state, and press one by index to change a mode
			response = _cmd_game_view(int(request.get("press", -1)))
		"get_scene_tree":
...
func _cmd_game_view(press: int) -> Dictionary:
	var game_view := _find_class(EditorInterface.get_base_control(), "GameView")
	if game_view == null:
		return {"error": "GameView not found"}
	var buttons: Array = []
	_collect_buttons(game_view, buttons)
	if press >= 0 and press < buttons.size():
		var b: BaseButton = buttons[press]
		if b.toggle_mode:
	...
	for i in buttons.size():
		var b: BaseButton = buttons[i]
		var entry := {"i": i, "class": b.get_class(), "tip": b.tooltip_text, "text": b.get("text") if "text" in b else "", "pressed": b.button_pressed, "disabled": b.disabled, "visible": b.is_visible_in_tree()}
		if b is MenuButton:
			var items: Array = []
			var popup: PopupMenu = (b as MenuButton).get_popup()
			for j in popup.item_count:
				items.append({"text": popup.get_item_text(j), "checked": popup.is_item_checked(j)})
			entry["items"] = items
		out.append(entry)
