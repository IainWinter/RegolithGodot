...
func _ready() -> void:
	# headless test runs never need the bridge, and a second instance must
	# not fail a GUT run with a port clash error
	if DisplayServer.get_name() == "headless":
		return

	# the bridge must answer while the tree is paused (pause menu, death)
	...
	if err != OK:
		print("MCP Bridge Game: could not listen on port %d (error %d)" % [port, err])
	else:
...
func _handle_request(client: StreamPeerTCP, data: String) -> void:
	...
	match cmd:
		...
		"action":
			# press an input action (e.g. pause, shoot) for hold seconds, so a
			# driver can reach what a click cannot
			var action_name: String = request.get("name", "")
			var hold: float = request.get("hold", 0.05)
			_send_response(client, _cmd_action(action_name, hold))
		_:
...
func _cmd_action(action_name: String, hold: float) -> Dictionary:
	if not InputMap.has_action(action_name):
		return {"error": "Unknown action: %s" % action_name}
	# an InputEventAction goes through the tree (_input / _unhandled_input /
	# is_action_just_pressed), a bare Input.action_press does not
	_send_action_event(action_name, true)
	_release_action_later(action_name, hold)
	return {"ok": true, "action": action_name, "hold": hold}
...
func _release_action_later(action_name: String, hold: float) -> void:
	await get_tree().create_timer(hold, true, false, true).timeout
	_send_action_event(action_name, false)
